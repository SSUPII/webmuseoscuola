			...
			<div class=choice>
				<form action=# method=post>
					<select name=uuid style="@charset "utf-8";">
						<?php
							$result = mysqli_query($connection,"SELECT id, username FROM users WHERE level = 0");
							if($result != FALSE && mysqli_num_rows($result) > 0){
					...
					<input type=submit name=promote value=<?php echo $strings[3]; ?> />
				</form>
				...
				<form action=# method=post>
					...
					<input type=submit name=demote value=<?php echo $strings[4]; ?> />
				</form>
			</div>
		</div>
    </body>
